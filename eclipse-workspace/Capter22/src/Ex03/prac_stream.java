package Ex03;

import java.util.Arrays;
import java.util.List;
import java.util.StringTokenizer;
import java.util.stream.IntStream;




class Customer {
    private String name;
    private int age;
    
    public Customer(String name, int age) {
        this.name = name;
        this.age = age;
    }
    public String getName() {
    	return name;
    }
    public int getAge() {
    	return age;
    }
}
public class prac_stream {

    public static void main(String[] args) {
        List<Integer> numbers = Arrays.asList(-3, -1, 0, 2, 5, -10, 7);
        numbers.stream()
        .filter(n -> (n > 0))
        .forEach(n -> System.out.println(n));
        
        List<String> st4 = Arrays.asList("java", "HTML", "CSS", "JavaScript");
        st4.stream()
               .map(s -> (s.length())) 
               .forEach(length -> System.out.println(length));
        
        int sents_counter = 0;
		String sent = "The quick Brown Fox";
		String[] sents = sent.split(" ");
		Arrays.stream(sents)
			.map(m -> (m.strip()) )
			.map(m -> (m.length()))
			.forEach(m -> System.out.println("단어토큰의 길이는" + m));
		
		
		

		List<Customer> customers = Arrays.asList(
			    new Customer("alice", 22),
			    new Customer("bob", 17),
			    new Customer("charlie", 25),
			    new Customer("david", 19),
			    new Customer("eve", 30)
			);
		customers.stream()
		.filter(n -> n.getAge()>=20)
		.sorted((s1,s2)->s1.getName().compareTo(s2.getName()))
		.map(n -> n.getName().toUpperCase())
		.forEach(n->System.out.print(n+" "));   	
	}
}
	    // 이름은 소문자 -> 대문자	   
		//	 getage와 getstring를이용하여 상수와 문자열으 ㄹ분리하자
	    //n은 이름에대응되는 나이 n<20
		// 위의정보를 알파벳순으로 정렬
		// 	알파벳배열에대해		.sorted()를이용하자

	    

		// 4. 고객 정보가 담긴 리스트가 있습니다. 각 고객은 이름(String)과 나이(int)를 
//		갖고 있습니다. 다음 조건에 따라 고객 이름 목록을 출력하는 프로그램을 작성하세요.
//		고객 중 20세 이상인 사람들만 필터링합니다.
//		이름을 알파벳 순으로 정렬합니다.
//		이름을 대문자로 변환하여 출력합니다.

		
		
		
		
		
		
		
		
		
		
